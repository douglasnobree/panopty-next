...
const PowerCalculation = () => {
  ...
  const [allTariffModules, setAllTariffModules] = useState<{ label: string; value: string }[]>([])
  ...
  const [allDescription, setAllDescription] = useState<{ label: string; value: string }[]>([])
  const [moduleTypeIsLoading, setModuleTypeIsLoading] = useState(false)

  const handleTariffChange = (option: { label: string; value: string } | null) => {
    if (option?.label) {
  ...
  const handleDescricaoChange = (option: { label: string; value: string } | null, index: number) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const formattedTariffModules = tariffModules.data.data.map((tariff: any) => ({
        label: `${tariff.rate_value} - ${tariff.rate_type.type} (${tariff.month}/${tariff.year})`,
        value: tariff.id.toString(),
      }))
      const formattedDescription = allDescription.data.map((description: any) => ({
  ...
  const handleCreateDescricao = async (inputValue: string) => {
    ...
    const createConfirm = window.confirm(`Você deseja criar uma nova Descrição: ${inputValue}`)
    if (createConfirm) {
      await api
  ...
  const handleInputChange = (
    ...
    const qnt = typeof updatedLamps[index].quantidade === "number" ? updatedLamps[index].quantidade : 0
    const pwr = typeof updatedLamps[index].potencia === "number" ? updatedLamps[index].potencia : 0
    const losses = typeof updatedLamps[index].perdas === "number" ? updatedLamps[index].perdas : 0
    updatedLamps[index].potenciaTotal = (qnt * (pwr - losses)) / 1000
    updatedLamps[index].consumoMes = Math.round(updatedLamps[index].potenciaTotal * 12 * (days || 0))
    setLamps(updatedLamps)
  }
  ...
  return (
    <div className="bg-background">
      <main className="container mx-auto px-4 py-6 space-y-6" style={{ maxWidth: "var(--max-width)" }}>
        <Card className="shadow-sm border border-[var(--border)]">
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                <Button variant="ghost" onClick={() => router.back()} className="text-[var(--slate-12)]">
                  <ArrowLeft className="h-4 w-4" />
                </Button>
                <CardTitle className="text-xl font-semibold text-[var(--slate-12)]">Cálculo de QIP</CardTitle>
              </div>
              <Button
                onClick={t}
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2 text-[var(--slate-12)]">Módulo tarifário</label>
                <Select
        ...
        <Card className="shadow-sm border border-[var(--border)]">
          <CardHeader>
            <CardTitle className="text-xl font-semibold text-[var(--slate-12)]">Configuração das Lâmpadas</CardTitle>
          </CardHeader>
            ...
            <UITable>
              <TableHeader>
                <TableRow className="border-b border-[var(--border)]">
                  ...
                  <TableHead className="text-[var(--slate-12)]">DESCRIÇÃO</TableHead>
                  <TableHead className="text-[var(--slate-12)]">QUANTIDADE</TableHead>
                  <TableHead className="text-[var(--slate-12)]">POTÊNCIA UNIT.</TableHead>
                  <TableHead className="text-[var(--slate-12)]">PERDAS</TableHead>
                  <TableHead className="text-[var(--slate-12)]">POTÊNCIA TOTAL (KW)</TableHead>
                  <TableHead className="text-[var(--slate-12)]">CONSUMO (kWh/mês)</TableHead>
                  <TableHead className="text-[var(--slate-12)]"></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {lamps.map((lamp, i) => {
                  const qnt = typeof lamp.quantidade === "undefined" ? 0 : lamp.quantidade
                  const pwr = typeof lamp.potencia === "undefined" ? 0 : lamp.potencia
                  const losses = typeof lamp.perdas === "undefined" ? 0 : lamp.perdas
                  const totalPower = (qnt * (pwr - losses)) / 1000
                  const totalPowerFormatted = Number.parseFloat(totalPower.toFixed(2))
                  const consumoMes = Math.round(totalPower * 12 * (days || 0))
                  return (
                    <TableRow key={i} className="border-b border-[var(--border)]">
                      <TableCell className="text-[var(--slate-12)]">{i + 1}</TableCell>
                      <TableCell>
                        <CreatableSelect
                          ...
                          onChange={(option) => handleDescricaoChange(option, i)}
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => handleInputChange(i, e, "quantidade")}
                      ...
                      <TableCell className="text-[var(--slate-12)]">{totalPowerFormatted}</TableCell>
                      <TableCell className="text-[var(--slate-12)]">{consumoMes}</TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
